/**
 * title:com.skch.service.impl.GetStereoscopicAssessmentOfHealthStatusEssentialFattyAcidResult.java
 * date:2017年9月28日下午5:57:54
 * version:1.0
 * description:健康状况立体评估——必须脂肪酸
 * 一、计算症状计数：按照选项顺序依次是：A：0分，B：1分，C：2分，D:3分，相加得总分即本部分的症状计数。
	二、计算症状负担：本部分的症状负担=本部分的症状计数（即总分）除以本部分问题总数（即数字8）
 ...
 * description:
 */
public class GetStereoscopicAssessmentOfHealthStatusEssentialFattyAcidResult implements GetResultInter {
