 ...
 */
public class GetThyroidTestResult implements GetResultInter
{
    @Override
    public String getResult(Map<String,List<T_variable>> param) {
        ...
        String result = "";
        if(score >= 0 && score <= 7){
            result = "你的甲状腺可能轻度功能不足  “6周健康饮食疗法”：完成本书第三部分所列的6周计划";
        }else if (score >=8 && score <= 11){
            result = "你的甲状腺可能中度功能不足    自我护理：完成本书第三部分所列的6周计划同时根据第15章所推荐的自我护理方法优化甲状腺功能";
        } else if (score >= 12){
            result = "你的甲状腺重度功能不足     医疗护理：完成以上两项任务的同时，可以向一名内科医生寻求额外的帮助。本书第15章罗列了一些你必须同这名内科医生讨论的问题。";
        }
        return result;
    }
}
