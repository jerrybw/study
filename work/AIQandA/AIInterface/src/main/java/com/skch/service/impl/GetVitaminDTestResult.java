 ...
 */
public class GetVitaminDTestResult implements GetResultInter
{
    @Override
    public String getResult(Map<String,List<T_variable>> param) {
        ...
        String result = "";
        if(score >= 0 && score <= 6){
            result = "你可能患有轻度维生素D缺失。“6周健康饮食疗法”：完成本书第三部分所列的6周计划。";
        }else if (score >= 9){
            result = "你可能患有重度维生素D缺失。 医疗护理：完成以上任务的同时，可以向一名内科医生寻求额外的帮助。本书第15章罗列了一些你必须同这名内科医生讨论的问题。";
        }
        return result;
    }
}
