...
import com.jerry.work.dao.ConversationRepository;
import com.jerry.work.entity.Conversation;
import com.jerry.work.util.Decoder;
...
import static com.jerry.work.util.SendResult.*;
...
//     在Java类中创建 logger 实例
    ...
    /**
     * 保存对话
     * @param request 用于获取请求端ip
     * @param conversation 对话参数
     * @return
     */
...
//            if ()
    ...
    public String sendMsg(Conversation conversation){
        ...
        String result = "";
        if(!"".equals(data)) {
            result = HttpRequest.sendPost(robotUrl, "jsonstr={\n" +
                    "                    \"questionId\": \"111\",\n" +
                    "                    \"question\": \"\",\n" +
                    "                    \"answer\": {\n" +
                    "                        \"askId\": \"\",\n" +
